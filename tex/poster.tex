...
\begin{lemma}
  ...
  \(x^n - 1 = \prod_{i \mid n} \Phi_i(x)\) が成り立つ.
...
\begin{definition}[オイラーのトーシェント関数]\
  ...
  \[\phi(n) := \# \{a \in \mathbb{N} \mid 1 \leq a \leq n, gcd(a, n) = 1\}.\]
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    = \prod_{1 \leq j \leq r} p_j^{n_j-1}(p_j-1)\) が成り立つ.
    \(p_j \geq 5\) なる素因数が存在すれば, \(\phi(n) \geq 4\) となるため不適.
    よって, \(n\) は \(2\) と \(3\) 以外の素因数をもたない. \(n = 2^a3^b\) (\(a, b \geq 0\)) とおくと,
    \(a \geq 3\) のとき, \(\phi(n) \geq 4\), \(b \geq 2\) のとき, \(\phi(n) \geq 3\) より,
    \(a \leq 2 ~\text{かつ}~ b \leq 1\).
    これらを考慮すると, 条件を満たすのは \((a, b) = (0, 0), (1, 0), (0, 1), (2, 0), (1, 1)\).
    したがって, \(n = 1, 2, 3, 4, 6\).
  \end{proof}
...
\section{おわりに}
  ...
  以上で形式化を終えたが, 掲載したコードは完成形のほんの一部に過ぎない.
  形式化を困難にしている要因は, 一見自明に思える事実の証明や場合分けの膨大さにある.
  すべてのパターンを漏れなく網羅できる点は, Leanを用いる利点でもあり, 多大な労力を要する部分でもある.
  今回は有理数体上で考えたが, 実数体や複素数体へと拡張すれば, 代数的構造はより複雑になり,
  さらなる困難が予想される.

  {\footnotesize こちらはコードの完成形が掲載されているGithubページである. \quad}
  \qrcode[height=1.3cm]{https://github.com/xidbad/Graduate/blob/main/project/project.lean}
